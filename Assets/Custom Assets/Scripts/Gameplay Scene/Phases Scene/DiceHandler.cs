...
public class DiceHandler : MonoBehaviour
{
    ...
    //-------------------------------------------------- serialize fields
    [SerializeField] GameObject dice1_Pf, dice2_Pf;
    [SerializeField] Transform dice1InstPoint_Tf, dice2InstPoint_Tf;

    //-------------------------------------------------- public fields
    [ReadOnly] public List<GameState_En> gameStates = new List<GameState_En>();

    [ReadOnly] public int dice1Amount, dice2Amount, diceTotalAmount;

    //-------------------------------------------------- private fields
    GameObject dice1_GO, dice2_GO;
    Controller_Phases controller_Cp;
    NoticePanel notice_Cp;
    ...
    void SetComponents()
    {
        ...
        notice_Cp = controller_Cp.ui_panelCanvas_Cp.notice_Cp;
    }
    ...
    //-------------------------------------------------- Throw dice
    public void ThrowDice(int playerId_tp)
    {
        StartCoroutine(Corou_ThrowDice(playerId_tp));
    }

    IEnumerator Corou_ThrowDice(int playerId_tp)
    {
        //
        AddGameStates(GameState_En.DiceRollStarted);

        // show notice ui
        string cont_tp = playerId_tp == 0 ? FactionNames.p1Name : FactionNames.p2Name;
        cont_tp += " サイコロスロー";
        Hash128 hash_tp = HashHandler.RegRandHash();
        notice_Cp.SetTitle(cont_tp, 3f, () => { HashHandler.RemoveHash(hash_tp); });

        // cast a dice
        if (PhotonNetwork.IsConnectedAndReady)
        {
            dice1_GO = PhotonNetwork.Instantiate(dice1_Pf.name, dice1InstPoint_Tf.position, Random.rotation);
            dice2_GO = PhotonNetwork.Instantiate(dice2_Pf.name, dice2InstPoint_Tf.position, Random.rotation);
        }
        else
        {
