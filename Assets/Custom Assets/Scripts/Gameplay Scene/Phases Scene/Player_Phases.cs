...
public class Player_Phases : MonoBehaviour
{
    ...
    public TokensData tokensData = new TokensData();

    public MarkersData markersData = new MarkersData();
    ...
    {
        bUnitCardDatas = dataManager_Cp.psBattleUnitCardsData[playerID].unitCards;

        mUnitCardDatas = dataManager_Cp.psMihariUnitCardsData[playerID].unitCards;
    }
    ...
    void InitBattleUnits()
    {
        ...
        for (int i = 0; i < bUnit_Cps.Count; i++)
        {
            ...
            bUnit_Cps[i].SetHp(bUnitCardDatas[i].hp);
    ...
    void InitMihariUnits()
    {
        ...
        for (int i = 0; i < mUnit_Cps.Count; i++)
        {
            ...
            mUnit_Cps[i].SetHp(mUnitCardDatas[i].hp);
    ...
    public void RefreshBattleInfo()
    {
        ...
        battleInfo.fushi = 0;
        battleInfo.ryu = 0;
        for (int i = 0; i < bUnitCardDatas.Count; i++)
        {
            if (bUnitCardDatas[i].attrib.Contains(UnitAttribute.Ken))
            {
                battleInfo.ken++;
            }
            if (bUnitCardDatas[i].attrib.Contains(UnitAttribute.Ma))
            {
                battleInfo.ma++;
            }
            if (bUnitCardDatas[i].attrib.Contains(UnitAttribute.Yumi))
            {
                battleInfo.yumi++;
            }
            if (bUnitCardDatas[i].attrib.Contains(UnitAttribute.Fushi))
            {
                battleInfo.fushi++;
            }
            if (bUnitCardDatas[i].attrib.Contains(UnitAttribute.Ryu))
            {
                battleInfo.ryu++;
            }
        }
    ...
    {
        //******************** 11th timing. str phase after setting atk token
        unitAbil_Cp.HandleGameEventsTiming(unitAbil_Cp.oriUnit_Cp, GameEventsTiming.StrPhaseAtkSet);
        yield return new WaitUntil(() => unitAbil_Cp.ExistGameStates(
            UnitsAbilityHandler.GameState_En.GameEventsFinished));
        unitAbil_Cp.RemoveGameStates(UnitsAbilityHandler.GameState_En.GameEventsFinished);
    }
    ...
    IEnumerator Corou_PlayRound(int roundIndex)
    {
        ...
        yield return new WaitUntil(() => unitAbil_Cp.ExistGameStates(UnitsAbilityHandler.GameState_En.ActionFinished));
    ...
    IEnumerator Corou_FinishRound(int roundIndex)
    {
        ...
        yield return new WaitUntil(() => unitAbil_Cp.ExistGameStates(UnitsAbilityHandler.GameState_En.RoundFinished));
