...
public class Controller_BattlePhase : MonoBehaviour
{
    ...
    IEnumerator CorouPlayRound(int roundIndex)
    {
        ...
        yield return new WaitUntil(() => btlUI_Cp.ExistGameStates(UI_BattlePhase.GameState_En.Done_NoticeTxtShow));
        ...
        firstPlayer_Cp.HighlightRoundPanel(true, roundIndex);

        firstPlayer_Cp.PlayRound(roundIndex);
        ...
        firstPlayer_Cp.HighlightRoundPanel(false, roundIndex);

        // wait
        yield return new WaitForSeconds(1f);

        // snd player round play
        sndPlayer_Cp.HighlightRoundPanel(true, roundIndex);

        sndPlayer_Cp.PlayRound(roundIndex);
        ...
        sndPlayer_Cp.HighlightRoundPanel(false, roundIndex);

        // finish round
        firstPlayer_Cp.FinishRound(roundIndex);
        yield return new WaitUntil(() => firstPlayer_Cp.ExistGameStates(Player_Phases.GameState_En.RoundFinished));
        firstPlayer_Cp.RemoveGameStates(Player_Phases.GameState_En.RoundFinished);

        sndPlayer_Cp.FinishRound(roundIndex);
        yield return new WaitUntil(() => sndPlayer_Cp.ExistGameStates(Player_Phases.GameState_En.RoundFinished));
